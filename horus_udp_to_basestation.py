...
class HorusUDPToBasestation(object):
	...
	def handle_sonde_message(self, packet):		
		...
		# Output basestation
		basestation = "MSG,3,,,"+icao+",,"+timestamp1+","+timestamp2+","+timestamp1+","+timestamp2+","+str(callsign)+","+str(altitude)+","+str(speed)+","+str(heading)+","+str(latitude)+","+str(longitude)+","+str(vert_speed)+",,,,," 
		...
		with open(config.ICAO_FILE, 'r+') as file:
			...
			if callsign in icaos:
				...
				if len(icaos) >= config.ICAO_LIMIT:
					...
					sorted_icaos = sorted(icaos.items(), key=lambda x: datetime.datetime.strptime(x[1]['timestamp'], TIMESTAMP_FORMAT))
...
class Sonde(object):
	...
	def calculate_ascent_rate(self, altitude, time):
		...
		if self.last_alt != None and self.last_message_time != None:
			self._ascent_rates.append(int((altitude - self.last_alt) / ((time - self.last_message_time).total_seconds() / 60)))
